...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store token in cookie (simple client-side for now, would be HttpOnly in production)
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center overflow-hidden bg-background">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="particle absolute top-1/4 left-1/4 w-1 h-1 bg-purple-500/20 rounded-full" />
        <div className="particle absolute top-1/3 right-1/3 w-1.5 h-1.5 bg-cyan-400/15 rounded-full" />
      ...
      >
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex items-center gap-2 mb-6 group">
            <div className="w-10 h-10 rounded-xl bg-purple-600/10 border border-purple-500/20 flex items-center justify-center group-hover:bg-purple-600/20 transition-all">
              <Terminal className="w-5 h-5 text-purple-400" />
            </div>
            <span className="font-bold text-xl tracking-wider">GNONYMOUS</span>
          </Link>
          <h1 className="text-2xl font-bold tracking-tight">Intelligence OS Access</h1>
          <p className="text-[--text-3] text-sm mt-2">Enter credentials to authenticate session</p>
        </div>
